...
|名前|戻り値|
|:--|:--|
|`template <typename T> std::vector<T>`<br>`xor_convolution(std::vector<T> a, std::vector<T> b, const T id = 0);`|$A, B$ に対する添え字 xor での畳み込み|
...
|名前|戻り値|
|:--|:--|
|`template <typename T> std::vector<T>`<br>`gcd_convolution(std::vector<T> a, std::vector<T> b);`|$A, B$ に対する添え字 gcd での畳み込み|
...
|名前|戻り値|
|:--|:--|
|`template <typename T> std::vector<T>`<br>`lcm_convolution(std::vector<T> a, std::vector<T> b, const int n = -1);`|$A, B$ に対する添え字 lcm での畳み込み|
