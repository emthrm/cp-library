...
- 結合法則 $a + (b + c) = (a + b + c)$，
- 単位元 (零元) の存在 $\exists 0 \text{ s.t. } a + 0 = 0 + a = a$，
- 逆元 (反数) の存在 $\forall a,\ \exists {-a} \text{ s.t. } a + (-a) = (-a) + a = 0$，
- 交換法則 $a + b = b + a$
...
- 単位元の存在 $\forall a \in S,\ \exists e \in S \text{ s.t. } a \cdot e = e \cdot a = a$
...
- 零元の存在 $\exists 0 \text{ s.t. } a \cdot 0 = 0 \cdot a = 0$
