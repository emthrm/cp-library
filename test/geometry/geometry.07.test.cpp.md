...
data:
  ...
  bundledCode: "#line 1 \"test/geometry/geometry.07.test.cpp\"\n/*\r\n * @brief \u8A08\
    ...
    \ --x;\r\n    ++x;\r\n    if (x > i - 1) x = i - 1;\r\n    for (int j = 0; j <=\
    \ k; ++j) {\r\n      if (j + 1 <= k) {\r\n        if (dp[x][j][true] + right -\
    \ std::max(ball[x].second, left) > dp[i][j + 1][true]) {\r\n          dp[i][j\
    \ + 1][true] = dp[x][j][true] + right - std::max(ball[x].second, left);\r\n  \
    \      }\r\n        if (dp[x][j][false] + right - left > dp[i][j + 1][true]) {\r\
