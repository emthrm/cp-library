...
data:
  ...
  bundledCode: "#line 1 \"test/math/quadratic_equation.test.cpp\"\n/*\r\n * @brief\
    \ \u6570\u5B66/\u4E00\u5143\u4E8C\u6B21\u65B9\u7A0B\u5F0F\r\n */\r\n#define PROBLEM\
    \ \"http://judge.u-aizu.ac.jp/onlinejudge/description.jsp?id=1039\"\r\n\r\n#include\
    \ <algorithm>\r\n#include <cmath>\r\n#include <iostream>\r\n#include <tuple>\r\
    ...
    \  Real arg() const { Real res = std::atan2(y, x); return res < 0 ? res + PI *\
    \ 2 : res; }\r\n  Real norm() const { return x * x + y * y; }\r\n  Point rotate(Real\
    \ angle) const { Real cs = std::cos(angle), sn = std::sin(angle); return Point(x\
    \ * cs - y * sn, x * sn + y * cs); }\r\n  Point unit_vector() const { Real a =\
    \ abs(); return Point(x / a, y / a); }\r\n  std::pair<Point, Point> normal_unit_vector()\
    ...
    \n}\r\n}  // geometry\r\n#line 6 \"math/quadratic_equation.hpp\"\n\r\nstd::vector<double>\
    ...
    \ x2);\r\n  return {x1, x2};\r\n}\r\n#line 14 \"test/math/quadratic_equation.test.cpp\"\
    \n\r\nint main() {\r\n  constexpr double EPS = 1e-8;\r\n  constexpr int INF =\
    \ 0x3f3f3f3f;\r\n  while (true) {\r\n    int n, m;\r\n    std::cin >> n >> m;\r\
    \n    if (n == 0 && m == 0) break;\r\n    std::vector<std::pair<geometry::Point,\
    \ double>> dog;\r\n    for (int i = 0; i < n; ++i) {\r\n      geometry::Point\
    \ d;\r\n      double v;\r\n      std::cin >> d >> v;\r\n      dog.emplace_back(d,\
    ...
    \ t(n, INF);\r\n      for (int i = 0; i < n; ++i) {\r\n        geometry::Point\
    \ d;\r\n        double v;\r\n        std::tie(d, v) = dog[i];\r\n        std::vector<double>\
    \ ans = quadratic_equation(fv.norm() - v * v, geometry::dot(fp - d, fv) * 2, (fp\
    \ - d).norm());\r\n        while (!ans.empty()) {\r\n          if (ans.back()\
    \ >= 0) t[i] = ans.back();\r\n          ans.pop_back();\r\n        }\r\n     \
    \ }\r\n      double frisbee = *std::min_element(t.begin(), t.end());\r\n     \
    \ for (int i = 0; i < n; ++i) {\r\n        if (std::abs(t[i] - INF) < EPS) continue;\r\
  ...
  code: "/*\r\n * @brief \u6570\u5B66/\u4E00\u5143\u4E8C\u6B21\u65B9\u7A0B\u5F0F\r\
    ...
    \ i < n; ++i) {\r\n        geometry::Point d;\r\n        double v;\r\n       \
    \ std::tie(d, v) = dog[i];\r\n        std::vector<double> ans = quadratic_equation(fv.norm()\
    \ - v * v, geometry::dot(fp - d, fv) * 2, (fp - d).norm());\r\n        while (!ans.empty())\
    \ {\r\n          if (ans.back() >= 0) t[i] = ans.back();\r\n          ans.pop_back();\r\
    \n        }\r\n      }\r\n      double frisbee = *std::min_element(t.begin(),\
    \ t.end());\r\n      for (int i = 0; i < n; ++i) {\r\n        if (std::abs(t[i]\
    \ - INF) < EPS) continue;\r\n        if (std::abs(frisbee - t[i]) < EPS) ++ans[i];\r\
