...
#include "../../../graph/light/biconnected_component.hpp"
...
int main() {
  ...
  if (odd == 1) {
    for (const std::pair<int, std::vector<int>> &pr : mp) {
      ...
      std::vector<std::vector<int>> graph(vers.size());
      for (int i = 0; i < n; ++i) {
        std::vector<int> tmp;
        ...
        if (tmp.size() >= 3) {
          graph[tmp.front()].emplace_back(tmp.back());
          graph[tmp.back()].emplace_back(tmp.front());
        }
        tmp.clear();
