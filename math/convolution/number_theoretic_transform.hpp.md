...
data:
  ...
  code: "/**\r\n * @brief \u6570\u8AD6\u5909\u63DB\r\n * @docs docs/math/convolution/number_theoretic_transform.md\r\
    ...
    \     if (primes[i][0] == ModInt::get_mod()) {\r\n        n_max = 1 << primes[i][2];\r\
    \n        root = ModInt(primes[i][1]).pow((ModInt::get_mod() - 1) >> primes[i][2]);\r\
    \n        return;\r\n      }\r\n    }\r\n    assert(false);\r\n  }\r\n\r\n  template\
    ...
    \ std::vector<U> &a, const std::vector<U> &b) {\r\n    const int a_size = a.size(),\
    ...
    \n\r\n  void calc(std::vector<ModInt> &a) {\r\n    const int n = a.size(), prev_n\
    \ = butterfly.size();\r\n    if (n > prev_n) {\r\n      assert(n <= n_max);\r\n\
    \      butterfly.resize(n);\r\n      const int prev = omega.size(), lg = __builtin_ctz(n);\r\
    \n      for (int i = 1; i < prev_n; ++i) butterfly[i] <<= lg - prev;\r\n     \
    \ for (int i = prev_n; i < n; ++i) butterfly[i] = (butterfly[i >> 1] >> 1) | ((i\
    \ & 1) << (lg - 1));\r\n      omega.resize(lg);\r\n      for (int i = prev; i\
    \ < lg; ++i) {\r\n        omega[i].resize(1 << i);\r\n        ModInt tmp = root.pow((ModInt::get_mod()\
